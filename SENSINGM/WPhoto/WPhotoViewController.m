...
- (void)getHeightVersionAllPhotos {
    ...
    [WPFunctionView getHeightVersionAllPhotos:^(PHFetchResult *allPhotos) {
        ...
        if (!self.allPhotoArr) {
            self.allPhotoArr = [[NSMutableArray alloc]init];
        }
...
- (void)getLowVersionAllPhotos {
    ...
    [WPFunctionView getLowVersionAllPhotos:^(ALAssetsGroup *group) {
        if (!self.photoGroupArr) {
            self.photoGroupArr = [[NSMutableArray alloc]init];
        }
        
        if (group!=nil) {
            [self.photoGroupArr addObject:group];
        }
        else{
            ALAssetsGroup* allPhotoGroup = self.photoGroupArr[self.photoGroupArr.count-1];
            ...
            [allPhotoGroup enumerateAssetsUsingBlock:^(ALAsset *result, NSUInteger index, BOOL *stop) {
                ...
                if (result&&[[result valueForProperty:ALAssetPropertyType]isEqualToString:ALAssetTypePhoto]) {
                    ...
                    //照片内容url加入数组
                    [self.allPhotoArr addObject:result.defaultRepresentation.url];
                }
                else{
                    //刷新显示
                    if (self.allPhotoArr.count) {
                        for (NSInteger i = 0; i<self.allPhotoArr.count; i++) {
                            NSString *cellId = [NSString stringWithFormat:@"cell%ld", (long)i];
                            [self.ado_collectionView registerClass:[myPhotoCell class] forCellWithReuseIdentifier:cellId];
                        }
                        [self.ado_collectionView reloadData];
        ...
        [WPFunctionView getChoosePicPHImageManager:^(double progress) {
            ...
            if (!result) {
                ...
                if (cell.chooseStatus == NO) {
                    if ((self.chooseArray.count+self.choosePhotoArr.count)< self.selectPhotoOfMax) {
                        [self.chooseArray addObject:result];
                        [self.chooseCellArray addObject:[NSString stringWithFormat:@"%ld",(long)indexPath.row]];
                        [self finishColorAndTextChange:self.chooseArray.count+self.choosePhotoArr.count];
                    ...
                    for (NSInteger j = 0; j<self.chooseCellArray.count; j++) {
                        
                        NSIndexPath *ip = [NSIndexPath indexPathForRow:[self.chooseCellArray[j] integerValue] inSection:0];
                        
                        if (indexPath.row == ip.row) {
                            [self.chooseArray removeObjectAtIndex:j];
                        }
                    }
                    [self.chooseArray removeObject:result];
                    [self.chooseCellArray removeObject:[NSString stringWithFormat:@"%ld",(long)indexPath.row]];
                    [self finishColorAndTextChange:self.chooseArray.count+self.choosePhotoArr.count];
