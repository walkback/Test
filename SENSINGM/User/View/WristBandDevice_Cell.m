...
@interface WristBandDevice_Cell ()


@end

@implementation WristBandDevice_Cell
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier {
    if (self = [super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        [_step_imagev mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.mas_equalTo(self->_device_imagev);
            make.top.equalTo(line_imageview.mas_bottom).offset(15);
            make.size.mas_equalTo(CGSizeMake(16, 16));
            make.bottom.equalTo(content_view).offset(-15);
        }];
        
        _sleep_imagev = [UIImageView new];
        _sleep_imagev.image = [UIImage imageNamed:@"睡眠"];
        [content_view addSubview:_sleep_imagev];
        [_sleep_imagev mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self->_step_imagev.mas_right).offset( (WIDTH - 15 - 16 * 3) / 3);
            make.centerY.mas_equalTo(self->_step_imagev);
            make.size.mas_equalTo(CGSizeMake(16, 16));
        }];
        
        _heartbeat_imagev = [UIImageView new];
        _heartbeat_imagev.image = [UIImage imageNamed:@"心率"];
        [content_view addSubview:_heartbeat_imagev];
        [_heartbeat_imagev mas_makeConstraints:^(MASConstraintMaker *make) {
            make.left.equalTo(self->_sleep_imagev.mas_right).offset( (WIDTH - 15 - 16 * 3) / 3);
            make.centerY.mas_equalTo(self->_step_imagev);
            make.size.mas_equalTo(CGSizeMake(16, 16));
        }];
        ...
        [_stepcount_lab mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.right.equalTo(self->_sleep_imagev.mas_left).offset(-6);
        }];
        
        _sleeptime_lab = [UILabel new];
        _sleeptime_lab.font = [UIFont systemFontOfSize:14];
        _sleeptime_lab.textColor = TEXTCOLOR;
        [content_view addSubview:_sleeptime_lab];
        [_sleeptime_lab mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(self->_sleep_imagev).offset(-8);
            make.left.equalTo(self->_sleep_imagev.mas_right).offset(6);
            make.right.equalTo(self->_heartbeat_imagev.mas_left).offset(-6);
        }];
        
        _heartbeat_num_lab = [UILabel new];
        _heartbeat_num_lab.font = [UIFont systemFontOfSize:14];
        _heartbeat_num_lab.textColor = TEXTCOLOR;
        [content_view addSubview:_heartbeat_num_lab];
        [_heartbeat_num_lab mas_makeConstraints:^(MASConstraintMaker *make) {
            make.top.mas_equalTo(self->_heartbeat_imagev).offset(-8);
            make.left.equalTo(self->_heartbeat_imagev.mas_right).offset(6);
            make.right.equalTo(content_view).offset(-6);
        }];
        ...
        _sleeptime_update_lab = [UILabel new];
        _sleeptime_update_lab.font = [UIFont systemFontOfSize:12];
        _sleeptime_update_lab.textColor = [UIColor colorWithRed:204/255.0 green:204/255.0 blue:204/255.0 alpha:1.0];
        [content_view addSubview:_sleeptime_update_lab];
        [_sleeptime_update_lab mas_makeConstraints:^(MASConstraintMaker *make) {
            make.bottom.mas_equalTo(self->_sleep_imagev).offset(7);
            make.left.equalTo(self->_sleep_imagev.mas_right).offset(6);
            make.right.equalTo(self->_heartbeat_imagev.mas_left).offset(-6);
        }];
        
        _heartbeat_update_lab = [UILabel new];
        _heartbeat_update_lab.font = [UIFont systemFontOfSize:12];
        _heartbeat_update_lab.textColor = [UIColor colorWithRed:204/255.0 green:204/255.0 blue:204/255.0 alpha:1.0];
        [content_view addSubview:_heartbeat_update_lab];
        [_heartbeat_update_lab mas_makeConstraints:^(MASConstraintMaker *make) {
            make.bottom.mas_equalTo(self->_heartbeat_imagev).offset(7);
            make.left.equalTo(self->_heartbeat_imagev.mas_right).offset(6);
            make.right.equalTo(content_view).offset(-6);
        }];
