...
- (void)setupContent
{
    ...
    for (int i = 0; i < self.scrollView.subviews.count; i++) {
        ...
        imageView.tag = index;
//        NSLog(@"讨厌的结果 = %@",self.imagesArr[index]);
        [imageView sd_setImageWithURL:[NSURL URLWithString:self.imagesArr[index]]];
...
- (void)startTimer
{
    if (@available(iOS 10.0, *)) {
        self.timer = [NSTimer scheduledTimerWithTimeInterval:2.5 repeats:YES block:^(NSTimer * _Nonnull timer) {
